...
function sendMessage() {  
    ...
    const data = {  
        messages: messages.slice(-messageCount), // Ensure only the most recent messages are sent  
...
function checkInputRange(inputId, min, max) {  
    ...
    input.addEventListener('input', () => {  
        if (input.value < min || input.value > max) {  
            ...
            input.value = Math.min(Math.max(input.value, min), max); // Reset to valid range  
        }  
    });  
}  
